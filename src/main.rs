...
fn create_and_write(path: &std::path::Path, generated: String) {
    let mut file = match std::fs::File::create(path) {
        Err(why) => panic!("elitetopp: couldn't write to {}: {}", path.display(), why),
        Ok(file) => file
    };
...
fn main() {
    ...
    if cli_arguments.len() < 2 {
        println!("elitetopp - create c++ source from elite\n\
                 ----------\n\
    ...
    let z = format!("{}_out.cpp", cli_arguments.get(1).unwrap()).as_str().to_owned();
